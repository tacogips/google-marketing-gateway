...
`google-ads search --customer-id <digits> --query-file <path>`. Analytics Data
v1beta supports metadata, `runReport`, and `checkCompatibility` with the
...
`POST /v25/customers/<id>/googleAds:search`. Analytics Data is pinned to v1beta:
metadata is `GET`, while `runReport` and `checkCompatibility` are `POST` to the
fixed `analyticsdata.googleapis.com` origin.
...
AdMob, and Trends inventory and [the active implementation plan](impl-plans/completed/foundation-publisher-readers.md)
for delivery status.
