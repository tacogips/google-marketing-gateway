...
```bash
brew tap user/tap
brew install --cask google-marketing-gateway
```
...
swift run google-marketing-gateway --version | tail -n 1 | grep -Fx "$version"
```
...
```text
dist/homebrew-cask/google-marketing-gateway-<version>-darwin-arm64.dmg
dist/homebrew-cask/google-marketing-gateway-<version>-darwin-x64.dmg
```
...
```bash
version="$(tr -d '[:space:]' < VERSION)"
/Applications/Xcode.app/Contents/Developer/usr/bin/stapler validate "dist/homebrew-cask/google-marketing-gateway-${version}-darwin-arm64.dmg"
/Applications/Xcode.app/Contents/Developer/usr/bin/stapler validate "dist/homebrew-cask/google-marketing-gateway-${version}-darwin-x64.dmg"
spctl --assess --type open --context context:primary-signature --verbose=4 "dist/homebrew-cask/google-marketing-gateway-${version}-darwin-arm64.dmg"
spctl --assess --type open --context context:primary-signature --verbose=4 "dist/homebrew-cask/google-marketing-gateway-${version}-darwin-x64.dmg"
```
...
`../homebrew-tap/Casks/google-marketing-gateway.rb`.

After reviewing the rendered tap Cask:

```bash
cd ../homebrew-tap
git add Casks/google-marketing-gateway.rb README.md
git diff --staged --stat
git commit -m "chore: release google-marketing-gateway <version>"
...
```bash
brew fetch --cask user/tap/google-marketing-gateway
HOMEBREW_NO_GITHUB_API=1 brew audit --cask user/tap/google-marketing-gateway
```
...
verify `api/cask/google-marketing-gateway.json` from GitHub
