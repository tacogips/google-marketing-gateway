...
```bash
brew tap user/tap
brew install google-marketing-gateway
```
...
| --- | --- |
| macOS Apple Silicon | `google-marketing-gateway-<version>-darwin-arm64.tar.gz` |
| macOS Intel | `google-marketing-gateway-<version>-darwin-x64.tar.gz` |
...
```bash
version="$(tr -d '[:space:]' < VERSION)"
scripts/render-homebrew-formula.sh "$version" /path/to/homebrew-tap/Formula/google-marketing-gateway.rb
```
...
brew install user/tap/google-marketing-gateway
google-marketing-gateway --version
brew test user/tap/google-marketing-gateway
```
...
verify `api/formula/google-marketing-gateway.json` from
